...
var (
	...
	//账号是否合法验证,字母开头，允许5-20字节，允许字母数字下划线
	validAccountPattern = regexp.MustCompile("^[a-zA-Z][a-zA-Z0-9_]{4,20}$")
	...
	MessageTpl = map[string]string{
		...
		"Account":   "输入账号有误，必须以字母开头，只能包含字母、数字或下划线，请保持在5-20个字符之间",
		"WeakPwd":   "密码不符合要求，必须以字母开头，长度在6~18字符之间，只能包含字母、数字和下划线",
		"StrongPwd": "密码不符合要求，必须包含大小写字母和数字的组合，不能使用特殊字符，长度在8个字符以上",
		"CnChar":    "必须包含中文字符",
